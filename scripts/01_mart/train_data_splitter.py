...
s3_dir = str(root_dir / "s3storage/01_public/auto_essay_scorer_lab2/")
...
for i, (train_index, valid_index) in enumerate(skf.split(X.to_pandas(), y.to_pandas())):
    ...
    # ディレクトリ作成    
    if not os.path.exists(os.path.join(path_to.middle_files_dir, f'fold_{i}')):
        os.mkdir(os.path.join(path_to.middle_files_dir, f'fold_{i}'))

    # CSVファイルとして保存
    train_fold_df.write_csv(os.path.join(path_to.middle_files_dir, f'fold_{i}', f'train_fold.csv'))
    valid_fold_df.write_csv(os.path.join(path_to.middle_files_dir, f'fold_{i}', f'valid_fold.csv'))