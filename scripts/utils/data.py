...
class CreateDataset:
    ...
    def Paragraph_Preprocess(self, tmp):
        ...
        tmp = tmp.with_columns(
            pl.col("paragraph").map_elements(self.remove_punctuation).alias("paragraph_no_pinctuation")
        )
        tmp = tmp.with_columns(
            pl.col("paragraph_no_pinctuation").map_elements(self.count_spelling_errors).alias("paragraph_error_num")
        )
    ...
    def Sentence_Preprocess(self, tmp):
        ...
        tmp = tmp.with_columns(
            pl.col("full_text").map_elements(self.dataPreprocessing).str.split(by=".").alias("sentence")
        )
    ...
    def deberta_oof_scores(self, input_data):
        ...
        def predict_scores(models, tokenizer, ds):
            ...
            args = TrainingArguments(".", per_device_eval_batch_size=self.config["EVAL_BATCH_SIZE"], report_to="none")

            predictions = []
            for model_path in models:
                model = AutoModelForSequenceClassification.from_pretrained(model_path)
                trainer = Trainer(
                    model=model, args=args, data_collator=DataCollatorWithPadding(tokenizer), tokenizer=tokenizer
                )
